...
import { InserterBuilder, ITableClass } from "./inserter-builder";
...
export default class TsBuilder {
    ...
    public renderTableFile(folder: string): void {
        ...
        const content = meta + start + arr.join("\n") + "\n}";
    ...
    public renderClassFiles(folder: string, searchString?: string) {
        ...
        tableClasses.forEach(tc => {
            const definition = interfaceBuilder.renderTs(tc.tableName, this.schema.tables[tc.tableName]);
    ...
    public renderViewClassFiles(folder: string, searchString?: string) {
        ...
        this.renderClasses(tables, folder).forEach(tc => {
            const definition = interfaceBuilder.renderTs(tc.tableName, this.schema.views[tc.tableName]);
            writeFileSync(tc.fullPath, definition);
        });
    }
    ...
    private renderClasses(tables: string[], folder: string): ITableClass[] {
        return tables.map(t => {
            let fnName: string;
            let fnPlural: string;
